...
test("runs clean, mismatch, other-provider comparison and missing-field replays", async ({
  page,
}) => {
  ...
  await page
    .getByText("Missing purchase-order number", { exact: true })
    .click();
  await page.getByRole("button", { name: "Run assurance check" }).click();
...
test("custom upload validation recovers through recorded replay", async ({
  page,
}) => {
  await page.goto("/workbench");
  ...
  await page
    .getByRole("button", { name: /Use a synthetic recorded replay/i })
    .click();
