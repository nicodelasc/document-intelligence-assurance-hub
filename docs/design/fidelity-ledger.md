...
Above-the-fold copy diff: the route uses `Operations` instead of the concept-only `Operations console`. It adds the mandatory keyless notice and labels the benchmark source as recorded. Concept-only help and reviewer controls were removed.
