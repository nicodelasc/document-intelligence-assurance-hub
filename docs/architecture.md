...
`GET /api/models` returns the server-owned catalogue, provider defaults and one boolean for each provider's availability. It returns no credential material. OpenAI and Anthropic keys stay inside the server container. Configuration does not establish acceptance. Local acceptance is mocked. Six controlled production requests were dispatched with zero provider retries: five used OpenAI GPT-5.6 Luna and one used Anthropic Claude Haiku 4.5. Four OpenAI runs failed closed before the selection and OCR bundle defects were corrected. The accepted original PDF completed OpenAI `Clear` and the accepted unverified PNG completed Anthropic `Conflict` with a review-only action. Conservative settled spend is US$0.0128216. Connected evidence now covers these two bounded provider routes while broader production acceptance remains gated.
